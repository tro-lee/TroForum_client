...
const TopicPostBubble = (props: any) => {
    ...
    const scroll = () => {
        ...
        if (
            ...
            if (page.current === page.pageNum) {
                if (
                    // @ts-ignore
                    slider.current.scrollTop > itemHeight * listData.length - screenH + 600
                ) {
                    // @ts-ignore
                    slider.current.scrollTop = itemHeight * listData.length - screenH + 400;
                }
            } else {
                setLoading(true);
    ...
    function getColorCode(num: number) {
        switch (num) {
            case 0:
                return "bg-blue-300";
            case 1:
            ...
            >
                ...
                >
                    {listData.slice(startIndex, endIndex).map((item, index) => {
                        ...
                        return (
                            <div
                                className={`border-t ${getColorCode(Number(item.postId.slice(-2)) % 21)} rounded-lg shadow-lg py-4 px-2 m-2`}
                                key={index}
                                style={{
                                    position: 'absolute',
                                    left: 0,
                                    ...
                                    width: '100%',
                                    transform: `translateY(${(startIndex + index) * itemHeight}px) translateX(${translateX})`,
                                }}
                                onClick={() => {
                            ...
                            >
                                <div className="flex justify-between items-center">
                                    <div className="ml-5 text-2xl inline-block text-gray-400">
                                        {item.userName}
                                    </div>
                                    <div className="text-sm inline-block text-white">
