...
const ReplyPostTemplate = (props: any) => {
  ...
  return (
    <div className="mt-4 bg-gray-100 p-2 rounded-lg hover:bg-gray-200">
      <img
        src={`https://api.multiavatar.com/${data.authorId + data.master}.png`}
        alt=""
        className="w-10 h-10 inline cursor-pointer"
        onClick={() => history.push('/person/' + data.authorId)}
      />
      <div
        className="cursor-pointer text-gray-400 float-right"
        onClick={() => {
            ...
            <div key={index} className="ml-5 mt-5 bg-gray-200 p-4 rounded-lg">
              <img
                src={`https://api.multiavatar.com/${
                  item.authorId + data.master
                }.png`}
                alt=""
                className="w-8 h-8 inline cursor-pointer"
                onClick={() => history.push('/person/' + item.authorId)}
              />
              <div
                className="cursor-pointer text-gray-400 float-right"
                onClick={() => {
              ...
              <div className="text-sm">回复给{item.ReplyName}</div>
            </div>
          );
        })
      }
